...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSString *OFSampleAppAPIKey = @"f149844b04633ee2127cb64b3aa90f31";
        NSString *OFSampleAppAPISharedSecret = @"a30423f6dcbf5bf9";
...
- (RACSignal *) signalFromAPIMethod:(NSString *)method
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        [[[successSignal
           // 4.1. Extract the second argument
           map:^id(RACTuple *tuple) {
               return tuple.second;
           }]
          // 4.2. transform the results
          map:block]
         subscribeNext:^(id x) {
             // 4.3. send the results to the subscribers
